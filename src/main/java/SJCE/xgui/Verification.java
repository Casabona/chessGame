...
public class Verification {
	/*
	 * ======================================================================== SCAN
	 * MOVE PATH AND ATTACK PATH
	 * ========================================================================
	 */
	public static final byte RANK_2 = 1;
	public static final byte RANK_7 = 6;
	private static final int OFF = BoardUI.OFF_BOARD;
	private static final int[] toGeneralBoard = { OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF,
			OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, 0, 1, 2, 3, 4, 5, 6, 7, OFF, OFF, OFF, OFF, 8,
			9, 10, 11, 12, 13, 14, 15, OFF, OFF, OFF, OFF, 16, 17, 18, 19, 20, 21, 22, 23, OFF, OFF, OFF, OFF, 24, 25,
			26, 27, 28, 29, 30, 31, OFF, OFF, OFF, OFF, 32, 33, 34, 35, 36, 37, 38, 39, OFF, OFF, OFF, OFF, 40, 41, 42,
			43, 44, 45, 46, 47, OFF, OFF, OFF, OFF, 48, 49, 50, 51, 52, 53, 54, 55, OFF, OFF, OFF, OFF, 56, 57, 58, 59,
			60, 61, 62, 63, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF,
			OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, };

	private static final int[] toTestBoard = { 26, 27, 28, 29, 30, 31, 32, 33, 38, 39, 40, 41, 42, 43, 44, 45, 50, 51,
			52, 53, 54, 55, 56, 57, 62, 63, 64, 65, 66, 67, 68, 69, 74, 75, 76, 77, 78, 79, 80, 81, 86, 87, 88, 89, 90,
			91, 92, 93, 98, 99, 100, 101, 102, 103, 104, 105, 110, 111, 112, 113, 114, 115, 116, 117, };
	...
	public int scanPath(int source, int destination, int direction, int limit) {
		...
		destination = (destination >= BoardUI.SQUARE_COUNT) ? Integer.MAX_VALUE : toTestBoard[destination];
		for (int i = source + direction, l = 0; true; i += direction, l++) {
			scanSquare = toGeneralBoard[i];
			scanPiece = (toGeneralBoard[i] == OFF_BOARD) ? PiecesUI.NO_PIECE : board[toGeneralBoard[i]];

			if (toGeneralBoard[i] == OFF_BOARD)
				return OFF_BOARD;
	...
	public int initialVerify(Move move, int type) {
		...
		if (PiecesUI.isPiece(PiecesUI.PAWN, piece)) {
			if (Math.abs(rankDisplacement) == 2) {
				if ((PiecesUI.getColor(piece) == PiecesUI.COLOR_WHITE) ? (BoardUI.getRank(source) != RANK_2)
						: (BoardUI.getRank(source) != RANK_7))
					return INVALID_MOVE;
				int direction = compare(fileDisplacement, rankDisplacement, 0, 0, 12, 0, -0, -0, -12, -0, 0);
				return combineResult(scanMove(source, destination, direction, 2), EMPTY_DESTINATION);
			}
			if ((PiecesUI.getColor(piece) == PiecesUI.COLOR_WHITE) ? (rankDisplacement != +1)
					: (rankDisplacement != -1))
				return INVALID_MOVE;

			int direction = compare(fileDisplacement, rankDisplacement, 0, 13, 12, 11, -0, -13, -12, -11, 0);
			...
			int direction = (Math.abs(fileDisplacement) > Math.abs(rankDisplacement))
					? compare(fileDisplacement, rankDisplacement, 0, 14, 0, 10, 0, -14, 0, -10, 0)
					: compare(fileDisplacement, rankDisplacement, 0, 25, 0, 23, 0, -25, 0, -23, 0);
			return combineResult(scanMove(source, destination, direction, 1), piece);
		} else if (PiecesUI.isPiece(PiecesUI.BISHOP, piece)) {
			int direction = compare(fileDisplacement, rankDisplacement, 0, 13, 0, 11, -0, -13, -0, -11, 0);

			return combineResult(scanMove(source, destination, direction), piece);
		} else if (PiecesUI.isPiece(PiecesUI.ROOK, piece)) {
			int direction = compare(fileDisplacement, rankDisplacement, 1, 0, 12, 0, -1, -0, -12, -0, 0);

			return combineResult(scanMove(source, destination, direction), piece);
		} else if (PiecesUI.isPiece(PiecesUI.QUEEN, piece)) {
			int direction = compare(fileDisplacement, rankDisplacement, 1, 13, 12, 11, -1, -13, -12, -11, 0);

			return combineResult(scanMove(source, destination, direction), piece);
		} else if (PiecesUI.isPiece(PiecesUI.KING, piece)) {
			if (type == Move.CASTLE_MOVE) {
				int direction = compare(fileDisplacement, rankDisplacement, 1, 0, 0, 0, -1, -0, -0, -0, 0);

				int result = (fileDisplacement < 0) ? scanMove(source, destination - 1, direction, 3)
						: scanMove(source, destination, direction, 2);
	...
	private int combineResult(int scanResult, int flag) {
		...
		if (scanPiece != PiecesUI.NO_PIECE) {
			if (flag == EMPTY_DESTINATION)
				return INVALID_MOVE;
			else if (PiecesUI.sameColor(flag, scanPiece))
				return INVALID_MOVE;
		}
		return VALID_MOVE;
	}

	private int compare(int fileDisplacement, int rankDisplacement, int fprz, int fprp, int fzrp, int fnrp, int fnrz,
			int fnrn, int fzrn, int fprn, int fzrz) {
		return Utility.compare(fileDisplacement, Utility.compare(rankDisplacement, fprp, fprz, fprn),
	...
	public int finalVerify(Move move) {
		...
		if (findAttacker(PiecesUI.indexOf(board, PiecesUI.KING[color]),
				PiecesUI.switchColor(color)) != PiecesUI.NO_PIECE)
			return INVALID_MOVE;
		return VALID_MOVE;
	}
	...
	private int findAttacker(int square, int color) {
		...
		for (int direction : directionList) {
			if (scanAttack(square, direction, 1) < 0)
				return PiecesUI.NO_PIECE;
			if (scanPiece == PiecesUI.KNIGHT[color])
				return scanPiece;
		}
		return PiecesUI.NO_PIECE;
	}
